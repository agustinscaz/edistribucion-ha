...
- El **IEE** (Impuesto Especial sobre la Electricidad, %) y el **IVA a aplicar (%)** de este CUPS, EN ESE ORDEN — igual que en una factura real: el IEE es la base imponible del IVA, no un recargo aparte que se suma al final (`total = base × (1 + IEE%) × (1 + IVA%)`). Por regla general se aplican sobre el coste de energía (los tres tipos de tarifa) Y sobre el término de potencia (base imponible = energía + potencia, antes de IVA), para que los sensores de coste se acerquen más a lo que pagas de verdad. Por defecto 5,11269632% (el tipo estatal fijo del IEE) y 21% (el tipo general de IVA vigente) — ajústalos si tu contrato tiene tipos distintos. **Ninguno** se aplica a la compensación de excedentes (es una bonificación que se resta, no un consumo). Instalaciones que actualizan desde una versión anterior reciben estos valores por defecto automáticamente en el primer arranque tras actualizar (sin tener que abrir Opciones) — si ya habías puesto un 0% a propósito para alguno de los dos, ese 0% se respeta.

  **Verifica el IEE contra una factura real antes de confiar en el 5,11269632% por defecto** (issue #30): existen exenciones legales de IEE que NO son automáticas (exigen inscripción específica) — la más relevante si tienes autoconsumo con excedentes es la del art. 94.9 de la Ley de Impuestos Especiales, que exime de IEE la energía compensada con el excedente horario. Si tu comercializadora te tiene inscrito en esa exención, tu factura real no mostrará IEE cobrado (mira el desglose de impuestos) y deberías poner 0% aquí — sin ella, el 5,11269632% por defecto sigue siendo el tipo estatal general vigente y aplicable a la inmensa mayoría de contratos.
...
- El **alquiler del equipo de medida** y la **financiación del bono social** (€/día cada uno) — los factura tu comercializadora, no e-distribución, así que no hay forma de leerlos en vivo: mira tu factura y ponlos a mano (son importes fijos por contrato). Por defecto 0 para no afectar a nadie que no los configure. Se suman al término de potencia DESPUÉS de aplicar el IEE, no antes: con una factura real se confirmó que la base imponible del IEE es solo energía + potencia, ninguno de estos dos conceptos lo lleva (sí llevan IVA, como el resto de la factura). Con esto, `estimado_..._con_potencia` puede cuadrar centavo a centavo con tu factura real en vez de solo aproximarla.
...
| `sensor.<cups>_coste_estimado_hoy` / `_semana` / `_mes` | **ESTIMACIÓN**, no el importe real de la factura: coste de energía (kWh × precio, según fija/tramos/pvpc) CON el IEE y el IVA de este CUPS aplicados EN ESE ORDEN — sigue sin término de potencia NI alquiler de equipos de medida (ver el siguiente sensor para incluirlos). Solo se crea si has configurado precio de energía para este suministro. Atributos `total_sin_impuestos`, `total_con_iee`, `iee_percent` e `iva_percent` con el desglose completo. El de "hoy" incluye además `ultimo_cambio`/`minutos_sin_cambiar` (ver nota de frescura más abajo); el de "mes" incluye `rango_real` (fechas reales que cubre el periodo, ver nota más abajo) |
| `sensor.<cups>_coste_hoy_con_potencia` / `_semana_con_potencia` / `_mes_con_potencia` / `_acumulado_del_año_con_potencia` | Como el sensor anterior, pero sumando también el término de potencia del mismo periodo (`termino_de_potencia_dia`/`_semana`/`_mes`/`_año`, con IEE+IVA aplicados y, si los has configurado, el alquiler del equipo de medida y la financiación del bono social — estos dos últimos con IVA pero sin IEE) — una única cifra de "coste de energía + potencia, con impuestos". Solo se crea si tienes precio de energía Y de potencia configurados; atributos `coste_energia`/`termino_potencia` con el desglose |
...
| `sensor.<cups>_termino_de_potencia_año` | Término de potencia acumulado en lo que va de año — meses completados (con el precio de potencia vigente cuando se cacheó cada uno) + el mes en curso en vivo. Solo si has puesto precio de potencia en las opciones de este CUPS |
| `sensor.<cups>_compensacion_por_excedentes_año` | Compensación por excedentes acumulada en lo que va de año — meses completados (con el precio de compensación vigente cuando se cacheó cada uno) + el mes en curso en vivo. Solo si has activado la compensación de excedentes |

Los tres sensores `_año` de arriba (coste, potencia, compensación) suman los meses ya cerrados a partir de estadísticas propias que Home Assistant guarda indefinidamente (no de volver a pedirle el histórico a e-distribución, que solo retiene granularidad diaria ~1-2 meses hacia atrás). Un mes anterior a que esta versión empezara a escribir esa estadística (instalación a mitad de año, o justo tras actualizar) cuenta como 0€ de forma honesta, no se inventa. Si vienes de una versión anterior o instalaste a mitad de año, el servicio `edistribucion.rellenar_historico` (más abajo) también rellena estas estadísticas de coste para meses pasados, no solo las de energía del Panel de Energía.
...
| `sensor.<cups>_termino_de_potencia_dia` / `_semana` / `_mes` | Solo si has puesto precio de potencia en las opciones de este CUPS — kW contratados reales (punta/valle) × precio €/kW/día CON el IEE y el IVA de este CUPS aplicados EN ESE ORDEN, un coste fijo que no depende del consumo |
| `sensor.<cups>_precio_venta_excedentes` | Solo si has activado la compensación de excedentes — el precio configurado (€/kWh) tal cual, SIN impuestos (la compensación no lleva IEE/IVA a propósito). Pensado para plantillas/integraciones externas que necesitan el precio unitario suelto, p.ej. para multiplicarlo por un kWh de OTRA fuente (un CT clamp en tiempo real, no la lectura oficial con retraso) |
| `sensor.<cups>_compensacion_por_excedentes_hoy` / `_semana` / `_mes` | Solo si has activado la compensación de excedentes para este suministro — kWh exportados × precio configurado. El de "hoy" incluye atributos `ultimo_cambio`/`minutos_sin_cambiar` (ver nota de frescura más abajo). Ver también `_año`, arriba |
| `sensor.<cups>_balance_neto_hoy` / `_ayer` / `_semana` / `_mes` / `_año` | Compensación por excedentes del periodo − coste total con potencia del mismo periodo — positivo si ese periodo "regaló" dinero (compensación > coste), negativo si se pagó neto: la pregunta que en la práctica más importa, sin tener que restar dos sensores a mano. Solo se crea si tienes coste de energía Y potencia Y compensación de excedentes configurados; atributos `compensacion`/`coste_con_potencia` con el desglose. El de "ayer" (a diferencia de "hoy") se calcula a partir de `bundle["month"]`, no de `consumption` — así aterriza siempre en un día ya cerrado aunque e-distribución todavía no haya procesado el de hoy (ver nota sobre latencia más abajo); expone `fecha_real` para confirmarlo |
...
- **Diagnósticos descargables**: Ajustes → Dispositivos y servicios → e-distribución → menú (⋮) → Descargar diagnósticos. Útil para adjuntar a un issue sin tener que copiar nada a mano (la dirección postal se redacta automáticamente). Incluye, por CUPS, el caché de acumulado del año (`year_to_date_completed_months`/`year_to_date_month_details`) — para diagnosticar un total anual raro sin tener que activar `logger.set_level: debug` ni forzar un reload a mano.
...
- **Servicio `edistribucion.rellenar_historico`**: rellena el histórico de estadísticas del Dashboard de Energía Y de coste (estas últimas alimentan los sensores `_año`, ver arriba) para meses ANTERIORES al actual (el relleno automático solo cubre el mes en curso) — útil recién instalada la integración, actualizando desde una versión sin estadísticas de coste, o si vienes de otra solución y quieres recuperar el histórico completo (hasta 36 meses hacia atrás).
