...
- **Panel "e-distribución" en el menú lateral**: se añade solo al configurar la integración, sin montar nada a mano. Por cada suministro, tarjetas Lovelace **nativas** de HA (no cajas de texto hechas a mano): una de resumen (potencia contratada, término de potencia, coste hoy/mes, excedentes, comparativa), una gráfica de energía de las últimas 48h, y una gráfica de coste estimado día a día del mes (con las estadísticas ya guardadas por el `recorder`) — con las mismas gráficas, temas e interactividad que el resto de tu dashboard.
