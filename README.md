...
| `sensor.<cups>_balance_neto_hoy` / `_semana` / `_mes` / `_año` | Compensación por excedentes del periodo − coste total con potencia del mismo periodo — positivo si ese periodo "regaló" dinero (compensación > coste), negativo si se pagó neto: la pregunta que en la práctica más importa, sin tener que restar dos sensores a mano. Solo se crea si tienes coste de energía Y potencia Y compensación de excedentes configurados; atributos `compensacion`/`coste_con_potencia` con el desglose |
