...
- El **alquiler del equipo de medida** (€/día) — lo factura tu comercializadora, no e-distribución, así que no hay forma de leerlo en vivo: mira tu factura y ponlo a mano (es un importe fijo por contrato, no cambia salvo que cambies de equipo). Por defecto 0 para no afectar a nadie que no lo configure. Se suma al término de potencia (con el mismo IEE/IVA aplicado) — con esto, `estimado_..._con_potencia` puede cuadrar centavo a centavo con tu factura real en vez de solo aproximarla.
...
| `sensor.<cups>_coste_hoy_con_potencia` / `_semana_con_potencia` / `_mes_con_potencia` / `_acumulado_del_año_con_potencia` | Como el sensor anterior, pero sumando también el término de potencia del mismo periodo (`termino_de_potencia_dia`/`_semana`/`_mes`/`_año`, que también lleva el IEE y el IVA aplicados y, si lo has configurado, el alquiler del equipo de medida) — una única cifra de "coste de energía + potencia, con impuestos". Solo se crea si tienes precio de energía Y de potencia configurados; atributos `coste_energia`/`termino_potencia` con el desglose |
